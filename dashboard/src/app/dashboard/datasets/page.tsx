...
// We'll use the user context instead of hardcoded mock data
...
export default function DatasetsPage() {
  ...
  useEffect(() => {
    ...
    // Set auth context for dataset service
    if (userContext) {
  ...
  const loadDatasets = useCallback(async () => {
    ...
    try {
      ...
      // Always filter by current user ID if available
      const userContext = getUserContext();
      if (userContext) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          >
            {user ? `${user.fullName.split(' ')[0]}'s ${t('datasets')}` : t('datasets')}
          </motion.h1>
          <motion.p
