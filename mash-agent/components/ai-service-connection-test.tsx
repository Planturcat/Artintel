...
export interface AIServiceConnectionTestProps {
  defaultEndpoint?: string;
  onConnectionChange?: (status: boolean) => void;
  showModels?: boolean;
}

export function AIServiceConnectionTest({
  defaultEndpoint = 'http://localhost:11434',
  onConnectionChange,
  showModels = true
}: AIServiceConnectionTestProps) {
  // State for connection testing
  const [isChecking, setIsChecking] = useState<boolean>(false);
  const [connectionResult, setConnectionResult] = useState<ConnectionTestResult | null>(null);
  const [serviceEndpoint, setServiceEndpoint] = useState<string>(defaultEndpoint);
  ...
  // Function to test connection to AI service
  const checkConnection = async () => {
    ...
    try {
      ...
      // Make request to AI service health endpoint
      const response = await fetch(`${serviceEndpoint}/api/version`, {
        method: 'GET',
      ...
      if (response.ok) {
        ...
        const result: ConnectionTestResult = {
          success: true,
          message: `Connected to AI service ${data.version || 'server'}`,
          responseTime
        };
        ...
        if (showModels) {
          try {
            const modelsResponse = await fetch(`${serviceEndpoint}/api/tags`, {
              method: 'GET',
              headers: { 'Content-Type': 'application/json' },
        ...
        <div className="space-y-2">
          ...
          <div className="flex space-x-2">
            <Input
              ...
              disabled={isChecking}
              aria-label="AI service endpoint URL"
            />
            <Button 
          ...
          <div className={`flex flex-col gap-2 p-4 rounded-md mt-2 ${
            ...
            {!isChecking && !connectionResult?.success && (
              <div className="text-sm mt-1 border-t border-red-800/30 pt-2">
                <p>Make sure your AI service is running and accessible at the specified endpoint.</p>
              </div>
            )}
          </div>
        )}
...
export default AIServiceConnectionTest;
